...
describe IssuesController, type: :controller do
  ...
  describe 'GET index' do
    before { get :index, journal_id: journal.id }
    it { expect(assigns(:issues)).to eq([issue])  }
    it { expect(response).to render_template("index") }
  end

  describe 'GET new' do
    before { get :new, journal_id: journal.id }
  ...
  describe 'POST create' do
    label       = "new_journal"
    description = "desc"
    before { post :create, journal_id: journal.id,
  ...
  describe 'GET edit' do
    before { get :edit, id: issue.id, journal_id: journal.id }
    it { expect(response).to render_template("edit") }
  end

  describe 'POST update' do
    label = "changed"
    before { post :update, id: issue.id, journal_id: journal.id, issue: { label: label } }
    it { expect(response).to redirect_to journal_issues_url(journal) }
    it { expect(Issue.find(issue.id).label).to eq(label) }
  end

  describe "DELETE destroy" do
    before { delete :destroy, id: issue.id, journal_id: journal.id }
    it { expect(response).to redirect_to journal_issues_url(journal) }
